...
## 1. Datamodell
- [ ] 1.1 Legg `guestbook_entries` i `src/db/schema.ts` (id, author_name 1–60 CHECK, body 1–2000 CHECK, status pending/published/hidden CHECK default pending, author_id uuid NULL FK auth.users ON DELETE SET NULL, created_at, updated_at)
- [ ] 1.2 `npm run db:generate` → commit migrasjon under `drizzle/`
- [ ] 1.3 Håndskriv `drizzle/rls/guestbook_entries.sql`: RLS på, SELECT USING (status='published'), ingen klient-INSERT/UPDATE/DELETE, BEFORE UPDATE-trigger for updated_at (idempotent)
- [ ] 1.4 Kjør RLS/trigger-SQL mot Supabase (DIRECT_URL) og verifiser at RLS er aktiv
