...
- `Pullquote` (lokal komponent — Archivo Black 26px sitat, sentrert, stempel-rød double-border, max-width 600px).
- Prose-mapping for HTML-elementer: `h2`, `h3`, `h4`, `p`, `a`, `blockquote`, `code`, `pre`, `ul`, `ol`, `li` — stylet med Newsreader 18px brødtekst, mono for `code`, dashed border-left på `blockquote`, ink-strong link-hover (fra global a-style i tokens.css).
...
- **THEN** elementet rendres som et sentrert Archivo Black 26px sitat med stempel-rød kant

#### Scenario: blockquote stylet med dashed border
- **WHEN** en post inneholder vanlig Markdown blockquote (`> tekst`)
- **THEN** elementet rendres med venstre dashed border og Newsreader italic
